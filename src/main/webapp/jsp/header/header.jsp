	...
	<nav class="navbar navbar-light bg-light">
		<div class="container">
			<a class="navbar-brand"
				href="${pageContext.request.contextPath}/controller?command=GO_TO_MAIN">
				<img src="image/logo.png" width="70" height="70">
			</a>
			<div class="d-grid gap-2 d-md-flex justify-content-md-end">
				<button class="btn btn-primary me-md-2" type="button"
					onclick="window.location.href='${pageContext.request.contextPath}/controller?command=GO_TO_MAIN'">${main}</button>
				...
				<c:choose>
					<c:when test="${empty role}">
						<div class="dropdown">
							<button class="btn btn-secondary dropdown-toggle" type="button"
								...
								aria-expanded="false">${loginButton}</button>
							<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
								<li><a class="dropdown-item"
									href="${pageContext.request.contextPath}/controller?command=ADMIN_LOGIN">${admin}</a></li>
								<li><a class="dropdown-item" href="#">${user}</a></li>
								<li><a class="dropdown-item" href="#">${applicant}</a></li>
							</ul>
							<button class="btn btn-secondary dropdown-toggle" type="button"
								id="dropdownMenuButton1" data-bs-toggle="dropdown"
								aria-expanded="false">${registration}</button>
							<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
								<li><a class="dropdown-item"
									href="${pageContext.request.contextPath}/controller?command=USER_REGISTRATION">${user}</a></li>
								<li><a class="dropdown-item" href="#">${applicant}</a></li>
							</ul>
						</div>
					</c:when>
					<c:when test="${role == 'admin'}">
						<div class="dropdown">
							...
							<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
								<li><a class="dropdown-item"
									href="${pageContext.request.contextPath}/controller?command=GO_TO_ADMIN_MAIN_PAGE">Main
										page</a></li>
								<li><a class="dropdown-item" href="#">User managment</a></li>
								<li><a class="dropdown-item"
									href="${pageContext.request.contextPath}/controller?command=LOGOUT">Logout</a></li>
							</ul>
						</div>
					</c:when>
				</c:choose>
				<div class="header-right">
					<form name="header" method="post" id="localeForm"
						action="${pageContext.request.contextPath}/controller">
						<input type="hidden" name="command" value="LANGUAGE_CHANGE" /> <input
							type="hidden" name="current_page"
							value="${pageContext.request.requestURL}"> <select
							class="form-select form-select-sm"
							aria-label=".form-select-sm example" name="language"
